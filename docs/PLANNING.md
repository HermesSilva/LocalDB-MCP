...
- [x] `vscode-extension/` — VS Code extension that registers the server through the MCP provider API (`vscode.lm.registerMcpServerDefinitionProvider`, VS Code ≥ 1.101), bundling the Windows binary. Packaged by `build-vsix.ps1` (repo root) as a platform-specific `.vsix` (`--target win32-x64`, since LocalDB is Windows-only), which bumps the extension's build segment on each run. The extension version line (starting at `1.0.0`) is deliberately independent from `Cargo.toml`: the crate version is what the `vX.Y.Z` tag and the MCP Registry publish, and the Marketplace only requires each upload to be strictly higher than the previous one. Marketplace publish is manual (`-Publish`) and needs a verified `hermessilva` publisher + Azure DevOps PAT.
- [x] `claude-plugin/` — self-contained Claude Code plugin (bundles the Windows binary directly), validated locally (`claude plugin validate --strict` + real `claude --plugin-dir` smoke test, both passing). Submitted 2026-07-01 via [platform.claude.com/plugins/submit](https://platform.claude.com/plugins/submit) for the `claude-community` marketplace review — **pending review**, not yet listed. Note: this targets `claude-community` (`anthropics/claude-plugins-community`), not `claude-plugins-official` — the official marketplace is curated at Anthropic's sole discretion with no application process (see `code.claude.com/docs/en/plugins#submit-your-plugin-to-the-community-marketplace`).

Exit criterion: `io.github.hermessilva/mssql-localdb-mcp` installable via the official MCP Registry, signed binary, no blocking SmartScreen warning. **Mostly met** — published and listed on the MCP Registry. Code signing still deferred to a future release; `claude-community` plugin marketplace submission made, pending Anthropic review (approved plugins sync into `marketplace.json` nightly, so check there once approved — there's no path to the curated `claude-plugins-official` marketplace at all, by design).
